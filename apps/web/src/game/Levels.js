...
export const LEVELS = Array.from({ length: TOTAL_LEVELS }, (_, idx) => {
  ...
  const tunnelGateCount = idx < 4 ? 0 : Math.min(1 + Math.floor((idx - 3) * 0.6), 4);
  ...
  const tunnelGapWidth = idx < 4
    ? 3.2
    : Number(Math.max(2.1, 3.4 - idx * 0.07 - Math.max(0, idx - 6) * 0.06).toFixed(2));
  const tunnelGapHeight = idx < 4
    ? 3.8
    : Number(Math.max(2.25, 3.65 - idx * 0.14 - Math.max(0, idx - 7) * 0.08).toFixed(2));
  const routeLength = Number(Math.min(52, 22 + idx * 1.7 + Math.max(0, idx - 5) * 0.9).toFixed(2));
  ...
  return {
    ...
    tunnelGapWidth,
    tunnelGapHeight,
    tunnelDepth: Number((1.0 + t * 0.55).toFixed(2)),
    tunnelFrameThickness: Number((0.42 + t * 0.12).toFixed(2)),
